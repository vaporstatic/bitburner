...
export async function main(ns) {
  ...
  let maxram = ns.getServerMaxRam(hserv);
  let usedram = ns.getServerUsedRam(hserv);
  ...
  let threads = ns.formatNumber((availram / div) / hmemdiv, "0a");
  ...
  for (var i = 1; i < tservs.length - 1; ++i) {
    ...
    if (!ns.hasRootAccess(target) && ns.getServerNumPortsRequired(target) <= execount) {
      if (ns.fileExists("BruteSSH.exe")) {
        ns.brutessh(target);
      }
      if (ns.fileExists("FTPCrack.exe")) {
  ...
  for (var j = 1; j < servs.length - 1; ++j) {
    ...
    var serv = s1.toString();
    if (!ns.hasRootAccess(serv) && ns.getServerNumPortsRequired(serv) >= execount) { //make sure you can hack this server if you haven't already
  ...
  for (var z = 1; z < servs.length - 1; ++z) {
    ...
    let s1 = servs[z].split(',');
    let serv = s1.toString();
    let maxram = ns.getServerMaxRam(serv);
    let usedram = ns.getServerUsedRam(serv);
    let availram = maxram - usedram;
    let threads = ns.formatNumber((availram / div) / memdiv, "0a");
  ...
  if (pcount > 0) {
    ...
    for (var int = 0; int < pservs.length; ++int) {
      ...
      let pserv = pservs[int];
      let maxram = ns.getServerMaxRam(pserv);
      let usedram = ns.getServerUsedRam(pserv);
      let availram = maxram - usedram;
      let threads = Math.floor((availram / div) / pmemdiv);
